...
export interface ClientEvents {
  ...
  [Events.MESSAGE_REACTION_ADD]: [MessageStruct, string, string];
  /** Emitted when a reaction is removed from a message. */
  [Events.MESSAGE_REACTION_REMOVE]: [MessageStruct, string, string];
  /** Emitted when messages are bulk deleted. */
  [Events.MESSAGE_DELETE_BULK]: [string[]];
  /** Emitted when a message is pinned. */
  [Events.MESSAGE_PIN]: [MessageStruct];
  /** Emitted when a message is unpinned. */
  [Events.MESSAGE_UNPIN]: [MessageStruct];
  ...
  [Events.USER_PLATFORM_WIPE]: [any];
  /** Emitted when data is appended to a message (e.g., embeds loaded). */
  [Events.MESSAGE_APPEND]: [MessageStruct, MessageStruct];
}

// Import Emoji type for event declaration
...
export abstract class BaseClient extends EventEmitter {
  ...
  constructor(options: clientOptions = {}) {
    ...
    if (options.rest?.instanceURL) {
      // Custom instance - warn but don't block
      this.debug(
        "Connecting to a custom Stoat instance. Compatibility with stoat-selfbot.js is not guaranteed.",
      );
    }
    ...
    this.on("error", (error) => {
      ...
      if (this.#useDefaultErrorHandler && this.listenerCount("error") === 1) {
        console.error(
          "[stoat-selfbot.js] Unhandled client error occurred. " +
