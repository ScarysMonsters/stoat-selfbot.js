...
export class MessageManager extends BaseManager<MessageStruct, APIMessage> {
  ...
  async send(content: MessageOptions | string): Promise<MessageStruct> {
    ...
    const resp = (await this.client.api.post(
      ...
      { ...content, attachments, embeds, nonce: UUID.generate() },
    )) as APIMessage;
  ...
  async bulkDelete(
    ...
    await this.client.api.delete(`/channels/${this.channel.id}/messages/bulk`, { ids });
  }
  ...
  async edit(
    ...
    await this.client.api.patch(`/channels/${this.channel.id}/messages/${id}`, options);
  }
  ...
  async search(
    ...
    const response = (await this.client.api.post(
      ...
      query as Required<MessageSearchOptions>,
    )) as APIMessage[];
